...
import StackVisualizer from '../components/visualization/components/StackVisualizer';
import { STACK_OPERATIONS, COMPARISON_TEMPLATES, STACK_QUIZ_QUESTIONS } from '../utils/stackTemplates';
...
const StackPage = () => {
  ...
  // Navigation State
  const [activeTab, setActiveTab] = useState(() => localStorage.getItem('stack_selected_tab') || 'theory');

  useEffect(() => {
    localStorage.setItem('stack_selected_tab', activeTab);
  }, [activeTab]);

  // Operations State
  const [selectedAlgoKey, setSelectedAlgoKey] = useState('stackArray');

  const currentTemplate = useMemo(() => {
    return STACK_OPERATIONS[selectedAlgoKey];
  }, [selectedAlgoKey]);
  ...
  const [speedMs, setSpeedMs] = useState(() => parseInt(localStorage.getItem('stack_speed') || '1200', 10));
  const [compileError, setCompileError] = useState(null);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    localStorage.setItem('stack_speed', speedMs.toString());
  }, [speedMs]);
  ...
  // ==========================================
  const [selectedCompCategory, setSelectedCompCategory] = useState('implementation');
  ...
  const compPair = useMemo(() => {
    if (selectedCompCategory === 'implementation') {
      return {
        algoA: { name: 'Array Stack', template: COMPARISON_TEMPLATES.arrayStack, time: 'O(1)', space: 'O(N)' },
        algoB: { name: 'Java Stack', template: COMPARISON_TEMPLATES.javaStack, time: 'O(1)', space: 'O(N)' }
      };
    } else {
      return {
        algoA: { name: 'Balanced Parentheses', template: COMPARISON_TEMPLATES.balancedParen, time: 'O(N)', space: 'O(N)' },
        algoB: { name: 'Next Greater Element', template: COMPARISON_TEMPLATES.nge, time: 'O(N)', space: 'O(N)' }
      };
    }
  ...
  const fetchPracticeData = async () => {
    try {
      ...
      const probList = probRes.data.data.filter(p => p.category === 'STACK');
  ...
  const handleQuizSubmit = async () => {
    let score = 0;
    STACK_QUIZ_QUESTIONS.forEach((q, idx) => {
    ...
    try {
      await api.post('/quiz/submit', {
        category: 'STACK',
        score: score,
        totalQuestions: STACK_QUIZ_QUESTIONS.length
      });
    } catch (e) {
    ...
    <div className="dsa-page">
      ...
      <header className="dsa-header">
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--text-primary)' }}>🥞 Stacks Module</span>
        </div>
      </header>
        ...
        {activeTab === 'theory' && (
          <div className="theory-scroll-pane" style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem', padding: '1rem' }}>
            <section className="theory-markdown-content" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '20px', padding: '2rem' }}>
              <div>
                <h1 style={{ fontWeight: 800, margin: '0 0 1rem 0' }}>The Stack Data Structure</h1>
                <p style={{ fontSize: '1rem', color: 'var(--text-secondary)', lineHeight: '1.7' }}>
                  A Stack is a linear data structure that follows the <strong>LIFO (Last In First Out)</strong> principle. 
                  Imagine a stack of dinner plates: the plate placed last on top is the first one removed.
                </p>
              </div>
              ...
              <div style={{ borderLeft: '4px solid var(--accent)', paddingLeft: '1.25rem', margin: '0.5rem 0' }}>
                ...
                <ul style={{ paddingLeft: '1.25rem', color: 'var(--text-secondary)', lineHeight: '1.6' }}>
                  <li><strong>Single Access Point:</strong> All insertion and deletion occur at the top pointer.</li>
                  <li><strong>LIFO Property:</strong> The element pushed last is popped first.</li>
                  <li><strong>Constant Time:</strong> Basic operations (push, pop, peek) take O(1) time.</li>
                </ul>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  <div style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid var(--border)' }}>
                    <h4 style={{ margin: '0 0 0.25rem 0', color: 'var(--accent)' }}>Push</h4>
                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Adds an element to the top. Checks for overflow first in fixed arrays.</p>
                  </div>
                  <div style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid var(--border)' }}>
                    <h4 style={{ margin: '0 0 0.25rem 0', color: 'var(--accent)' }}>Pop</h4>
                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Removes and returns the top element. Checks for underflow if empty.</p>
                  </div>
                  <div style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid var(--border)' }}>
                    <h4 style={{ margin: '0 0 0.25rem 0', color: 'var(--accent)' }}>Peek</h4>
                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Returns the top element without removing it.</p>
                  </div>
                  <div style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid var(--border)' }}>
                    <h4 style={{ margin: '0 0 0.25rem 0', color: 'var(--accent)' }}>isEmpty</h4>
                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Returns true if the top pointer is -1.</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <aside className="theory-sidebar-topics" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.25rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {Object.keys(STACK_OPERATIONS).map(key => (
                    <button
                      key={key}
          ...
          <div className="visualizer-tab-content" style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '1.5rem', height: '100%' }}>
            ...
            <section className="visualizer-workspace" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <div className="visualizer-canvas-card" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '260px' }}>
                <StackVisualizer stepInfo={currentStep} />
              </div>
        ...
        {activeTab === 'comparison' && (
          <div>
            <div className="visualizer-control-card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '0.75rem 1.5rem', marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button onClick={() => { setSelectedCompCategory('implementation'); stopCompPlay(); }} className={`ctrl-btn ${selectedCompCategory === 'implementation' ? 'active' : ''}`} style={{ padding: '0.4rem 1rem' }}>Implementation</button>
                <button onClick={() => { setSelectedCompCategory('application'); stopCompPlay(); }} className={`ctrl-btn ${selectedCompCategory === 'application' ? 'active' : ''}`} style={{ padding: '0.4rem 1rem' }}>Application</button>
              </div>
              ...
              <button onClick={handleStartComparison} disabled={compIsLoading} className="ctrl-btn active" style={{ background: 'var(--accent)', color: '#fff', border: 'none', padding: '0.4rem 1.25rem', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }}>
                {compIsLoading ? 'Comparing...' : '📊 Run Comparison'}
              </button>
            ...
            <div className="comparison-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
              <div className="comparison-column" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div style={{ height: '220px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.1)', borderRadius: '10px' }}>
                  <StackVisualizer stepInfo={activeStepA} />
                </div>
              ...
              <div className="comparison-column" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div style={{ height: '220px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.1)', borderRadius: '10px' }}>
                  <StackVisualizer stepInfo={activeStepB} />
                </div>
                <div style={{ height: '220px', borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--border)' }}>
        ...
        {activeTab === 'quiz' && (
          <div className="quiz-pane" style={{ maxWidth: '800px', margin: '0 auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '20px', padding: '2rem' }}>
              <h2 style={{ margin: '0 0 0.5rem 0', fontWeight: 800 }}>Stack Knowledge Check</h2>
              <p style={{ margin: '0 0 1.5rem 0', color: 'var(--text-secondary)' }}>Verify your understanding of LIFO ordering, complexity limits, and JVM stack mechanics.</p>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                {STACK_QUIZ_QUESTIONS.map((q, qIdx) => {
              ...
              <div style={{ marginTop: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                {!quizSubmitted ? (
                  <button 
                    ...
                    disabled={Object.keys(quizAnswers).length < STACK_QUIZ_QUESTIONS.length}
                  ...
                  <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', width: '100%', justifyContent: 'space-between' }}>
                    <div style={{ fontSize: '1rem', fontWeight: 'bold' }}>
                      Your Score: <span style={{ color: quizScore >= STACK_QUIZ_QUESTIONS.length / 2 ? 'var(--success)' : 'var(--error)' }}>{quizScore} / {STACK_QUIZ_QUESTIONS.length}</span>
                    </div>
                    <button 
